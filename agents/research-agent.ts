...
Prompt 38 + **48** + **50** + **57** + **58**: **Insightful + stable** — zero generic filler; **zero** leakage (no API/LLM/schema/meta in strings). **icp_fit_score**: strong named B2B + work email + buyer title → **80–93** when fit holds; never absurd lows on real leads. Exec + ICP + news + pains + angles + BANT evidence: **sharp, distinct, zero repeated phrasing** across fields. **reasoning_steps** = **human SDR prep** (mixed cadence, validate-on-call notes) — **not** parallel corporate bullets. **Hard-ban** wallpaper phrases ("scaling aggressively", "finance enablement", "hypergrowth", "moving fast" as filler, etc.) unless **verbatim** from a cited fact — replace with **this** company's specifics. **Swap-test** exec + ICP: must break if company name is swapped for a sector peer. **Prompt 58:** **Product-surface terms** from web/notes when real; **no circular** exec↔ICP; pains/angles = **operational hooks**. **Apply CAMPAIGN SDR VOICE + RESEARCH_VOICE**. When WEB_RESEARCH_DIGEST is present, **weave** concrete items into narrative fields — **plain sentences**; **no** "(inferred)", **no** "unknown" placeholders, **no** digest meta, **no** pasting section headers into JSON. JSON only.`;
